...
module TwitterHelper
  ...
  def tweet_button(item)
    ...
    escaped_text = if item.user.income_ids.include?(from)
                     URI.escape("#{item.name} [#{accounts[from]}] #{number_to_currency(item.amount)}")
                   elsif item.user.expense_ids.include?(to)
                     URI.escape("#{item.name} [#{accounts[to]}] #{number_to_currency(item.amount)}")
                   else
                     URI.escape("#{item.name} #{number_to_currency(item.amount)}")
                   end
