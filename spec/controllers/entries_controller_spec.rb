        ...
        context "create adjustment to the same day as another ajustment's one," do
          ...
          context "input values are invalid," do
            ...
            describe "all adjustments count" do
              specify {
                expect { action.call }.not_to change{ Item.find_all_by_adjustment(true).count }
              }
            end
      ...
      describe "update adjustment" do
        ...
        context "when there is no future adjustment," do 
          ...
          describe "updated item" do
            ...
            specify {
              expect{@action.call}.not_to change{Item.find(items(:adjustment6).id).adjustment?}
            }
            specify {
        ...
        context "when change amount the adjustment which has an adjustment in the next month" do 
          ...
          describe "updated item" do
            ...
            specify {
              expect{@action.call}.not_to change{Item.find(items(:adjustment4).id).adjustment?}
            }
            specify {
        ...
        context "when change date," do
          ...
          describe "updated item" do
            ...
            specify {
              expect{@action.call}.not_to change{item.adjustment?}
            }
            specify {
        ...
        context "when account_id is changed," do
          ...
          before do
            ...
            xhr :post, :create, :entry_type => 'adjustment', :action_year=>old_adj6.action_date.year, :action_month=>old_adj6.action_date.month, :action_day=>old_adj6.action_date.day, :to => 13,:adjustment_amount => '1000', :year=>old_adj4.action_date.year, :month=>old_adj4.action_date.month
            ...
            @action = lambda { xhr :put, :update, entry_type: 'adjustment', id: items(:adjustment2).id, action_year: date.year, action_month: date.month, action_day: date.day, adjustment_amount: '3,000', to: 13, year: date.year, month: date.month }
          end
          ...
          describe "updated adjustment's amount change" do
            specify {
              ...
              total = scoped_items.where(to_account_id: 13).sum(:amount) - scoped_items.where(from_account_id: 13).sum(:amount)
              expect { @action.call }.to change { Item.find(adj2_id).amount }.from(@old_adj2.amount).to(3000 - total)
            }
          end

          describe "the future adjustment which has an old account_id" do
            specify {
              expect { @action.call }.to change { Item.find(adj4_id).amount }.by(@old_adj2.amount)
            }
          end
          ...
          describe "the future adjustment which has an new account_id" do
            specify {
              scoped_items = Item.where("action_date < ?", @date)
              total = scoped_items.where(to_account_id: 13).sum(:amount) - scoped_items.where(from_account_id: 13).sum(:amount)
              expect { @action.call }.to change { Item.find(@future_adj.id).amount }.by(total - 3000)
            }
          end
          ...
          describe "profit losses" do
            describe "pl of old account" do
              specify {
                expect { @action.call }.not_to change { MonthlyProfitLoss.find(@old_mpl.id).amount }
              }
            end
            ...
            describe "pl of new account" do
              specify {
                scoped_items = Item.where("action_date < ?", @date)
                total = scoped_items.where(to_account_id: 13).sum(:amount) - scoped_items.where(from_account_id: 13).sum(:amount)
                expect { @action.call }.to change { MonthlyProfitLoss.where(month: @old_adj2.action_date.beginning_of_month, account_id: 13).first.try(:amount).to_i }.by(3000 - total)
              }
            end
            ...
            describe "pl of new account" do
              specify {
                scoped_items = Item.where("action_date < ?", @date)
                total = scoped_items.where(to_account_id: 13).sum(:amount) - scoped_items.where(from_account_id: 13).sum(:amount)
                expect { @action.call }.to change { MonthlyProfitLoss.find(@new_adj_mpl.id).amount }.by(total - 3000)
              }
            end
          end
        end
        ...
        context "when action_date is changed to the next month and before adjustment," do
          let(:date) { items(:adjustment6).action_date - 1 }
          before do
            @action = lambda { xhr :put, :update, :entry_type => 'adjustment', :id => items(:adjustment2).id, :action_year => date.year, :action_month => date.month, :action_day => date.day, :adjustment_amount => '3,000', :to => items(:adjustment2).to_account_id, :year => date.year, :month => date.month
}
          end
          ...
          describe "other adjustments" do
            describe "adjustment4" do
              specify {
                old_adj2 = items(:adjustment2)
                expect { @action.call }.to change { Item.find(items(:adjustment4).id).amount }.by(old_adj2.amount)
              }
            end
            ...
            describe "adjustment6" do
              specify {
                old_adj6 = items(:adjustment6)
                expect{ @action.call }.to change{ Item.find(items(:adjustment6).id).amount }.to(old_adj6.adjustment_amount - 3000)
              }
            end
          end
          ...
          describe "balance" do
            specify {
              expect {@action.call}.not_to change { Account.asset(users(:user1), items(:adjustment2).to_account_id, items(:adjustment4).action_date)}
            }
            
            specify {
              expect {@action.call}.not_to change { Account.asset(users(:user1), items(:adjustment2).to_account_id, items(:adjustment6).action_date)}
            }
          ...
          describe "profit losses" do
            specify {
              expect {@action.call}.not_to change { MonthlyProfitLoss.where(month: Date.new(2008,2), account_id: items(:adjustment2).to_account_id).first.amount }
            }
            specify {
              expect {@action.call}.not_to change { MonthlyProfitLoss.where(month: Date.new(2008,3), account_id: items(:adjustment2).to_account_id).first.amount }
            }
          end
        end
      ...
      describe "update item" do
        ...
        describe "update without change month" do
          ...
          context "when there are adjustment in the same and future month," do
            ...
            describe "adjustment which is in the next month or after" do 
              ...
              specify {
                expect { @action.call }.not_to change{ Item.find(id4).amount}
              }
              specify {
                expect { @action.call }.not_to change{ Item.find(id6).amount}
              }
            ...
            describe "profit losses of the months before the updated item" do
              ...
              specify {
                expect { @action.call }.not_to change{ MonthlyProfitLoss.find(in200712_id).amount}
              }
              specify {
                expect { @action.call }.not_to change{ MonthlyProfitLoss.find(in200801_id).amount}
              }
              specify {
                expect { @action.call }.not_to change{ MonthlyProfitLoss.find(out200712_id).amount}
              }
              specify {
                expect { @action.call }.not_to change{ MonthlyProfitLoss.find(out200801_id).amount}
              }
            end
            ...
            describe "profit loss whose account has some adjustments in the same month (> day) as the updated item" do
              ...
              specify {
                expect { @action.call }.not_to change{ MonthlyProfitLoss.find(in200802_id).amount}
              }
            end
            ...
            describe "profit loss of the future months" do
              context "when profit loss exists," do 
                ...
                specify {
                  expect { @action.call }.not_to change{ MonthlyProfitLoss.find(in200803_id).amount}
                }
              end
        ...
        context "when updated item's action date changed from before-adj2 to after-adj4 but month of action_date doesn't change," do
          ...
          describe "adjustment changes" do 
            ...
            describe "adj2" do
              ...
              specify {
                expect{@action.call}.to change{Item.find(adj2_id).amount}.by( -1 *  @old_item1.amount)
              }
            end
            ...
            describe "adj4" do
              ...
              specify {
                expect{@action.call}.not_to change{Item.find(adj4_id).amount}
              }
            end
            ...
            describe "adj6" do
              before { @old_adj4 = items(:adjustment4) }
              specify {
                expect{@action.call}.to change{Item.find(adj6_id).amount}.by(20000)
              }
            end
          end
          ...
          describe "profit losses" do 
            ...
            specify {
              expect {@action.call}.not_to change{MonthlyProfitLoss.find(@old_pl200712.id).amount}
            }

            specify {
              expect {@action.call}.not_to change{MonthlyProfitLoss.find(@old_pl200801.id).amount}
            }
            
            specify {
              expect {@action.call}.to change{MonthlyProfitLoss.find(@old_pl200802.id).amount}.by(-20000)
            }
            
            specify {
              expect {@action.call}.to change{MonthlyProfitLoss.find(@old_pl200803.id).amount}.by(20000)
            }
          end
        end
        ...
        describe "updating credit item" do
          context "with same accounts, same month," do 
            before do
              _login_and_change_month(2008,2)
              xhr(:post, :create, action_year: '2008', action_month: '2', action_day: '10',
              ...
              @action = lambda {
                xhr(:put, :update, id: init_credit_item.id, item_name: 'テスト10',
                    action_year: date.year.to_s, action_month: date.month.to_s,
                    action_day: date.day.to_s, amount: "20000", from: accounts(:credit4).id.to_s,
            ...
            describe "monthly profit losses" do
              specify {
                expect { @action.call }.to change{MonthlyProfitLoss.where(account_id: 4, month: Date.new(2008,2,1)).first.amount}.by(-10000)
              }
              specify {
                expect { @action.call }.to change{MonthlyProfitLoss.where(account_id: 3, month: Date.new(2008,2,1)).first.amount}.by(10000)
              }
              specify {
                expect { @action.call }.to change{MonthlyProfitLoss.where(account_id: 4, month: Date.new(2008,4,1)).first.amount}.by(10000)
              }
              specify {
                expect { @action.call }.to change{MonthlyProfitLoss.where(account_id: 1, month: Date.new(2008,4,1)).first.amount}.by(-10000)
              }
            end
          end
          ...
          context "with same accounts, changed month," do 
            before do
              ...
              @action = lambda {
                xhr(:put, :update, id: init_credit_item.id, item_name: 'テスト10',
                    action_year: "2008", action_month: "3",
                    action_day: "10", amount: "20000", from: accounts(:credit4).id.to_s,
                    to: accounts(:outgo3).id.to_s, year: init_credit_item.action_date.year,
                    month: init_credit_item.action_date.month) }
            end
            ...
            describe "monthly profit losses" do
              describe "credit prev action_date's month" do 
                specify {
                  expect { @action.call }.to change{MonthlyProfitLoss.where(account_id: 4, month: Date.new(2008,2,1)).sum(:amount)}.by(10000)
                }
                specify {
                  expect { @action.call }.to change{MonthlyProfitLoss.where(account_id: 3, month: Date.new(2008,2,1)).sum(:amount)}.by(-10000)
                }
              end
              
              describe "payment prev action_date's month" do 
                specify {
                  expect { @action.call }.to change{MonthlyProfitLoss.where(account_id: 4, month: Date.new(2008,4,1)).sum(:amount)}.by(-10000)
                }
                specify {
                  expect { @action.call }.to change{MonthlyProfitLoss.where(account_id: 1, month: Date.new(2008,4,1)).sum(:amount)}.by(10000)
                }
              end
              describe "credit new action_date's month" do 
                specify {
                  expect { @action.call }.to change{MonthlyProfitLoss.where(account_id: 4, month: Date.new(2008,3,1)).sum(:amount)}.by(-20000)
                }
                specify {
                  expect { @action.call }.to change{MonthlyProfitLoss.where(account_id: 3, month: Date.new(2008,3,1)).sum(:amount)}.by(20000)
                }
              end
              
              describe "payment new action_date's month" do 
                specify {
                  expect { @action.call }.to change{MonthlyProfitLoss.where(account_id: 4, month: Date.new(2008,5,1)).sum(:amount)}.by(20000)
                }
                specify {
                  expect { @action.call }.to change{MonthlyProfitLoss.where(account_id: 1, month: Date.new(2008,5,1)).sum(:amount)}.by(-20000)
                }
              end
            end
          end
          ...
          context "with other accounts, same month," do
            before do
              ...
              @action = lambda {
                xhr(:put, :update, id: init_credit_item.id, item_name: 'テストUpdate10',
                    action_year: "2008", action_month: "2", action_day: "10",
            ...
            describe "updated item" do
              ...
              specify {
                expect { @action.call }.not_to change{Item.find(@credit_id).action_date}
              }
              specify {
                expect { @action.call }.to change{Item.find(@credit_id).from_account_id}.to(1)
              }
              specify {
                expect { @action.call }.to change{Item.find(@credit_id).to_account_id}.to(13)
              }
            end
            
            describe "payment item" do
              specify {
                expect { @action.call }.to change{Item.find_by_id(@payment_id)}.to(nil)
              }
            end
            ...
            describe "adjustment" do
              specify {
                expect { @action.call }.to change{Item.find(items(:adjustment2).id).amount}.by(20000)
              }
            end
            ...
            describe "monthly profit losses" do
              ...
              specify {
                expect { @action.call }.to change{MonthlyProfitLoss.where(account_id: 13, month: Date.new(2008,2,1)).sum(:amount)}.by(20000)
              }
              specify {
                expect { @action.call }.not_to change{MonthlyProfitLoss.where(account_id: 1, month: Date.new(2008,2,1)).sum(:amount)}
              }
            end
          end
        end
