  ...
  describe "#link_to_confirmation_required" do
    ...
    context "when neither tag nor mark nor keyword is specified," do
      context "when now confirmation_required is true," do
        ...
        it { should ==  link_to(I18n.t('label.confirmation_mark').html_safe, entry_confirmation_required_path(@item.id, confirmation_required: false), remote: true, method: :put, class: 'item_confirmation_required') }
      end

      context "when now confirmation_required is false," do
        subject { helper.link_to_confirmation_required(@item.id, false) }
        it { should ==  link_to(I18n.t('label.no_confirmation_mark').html_safe, entry_confirmation_required_path(@item.id, confirmation_required: true), remote: true, method: :put, class: 'item_confirmation_not_required') }
      end
    end
    ...
    context "when tag is specified," do
      context "when now confirmation_required is true," do
        ...
        it { should ==  link_to(I18n.t('label.confirmation_mark').html_safe, tag_entry_confirmation_required_path("tagtag", @item.id, confirmation_required: false), remote: true, method: :put, class: 'item_confirmation_required') }
      end

      context "when now confirmation_required is false," do
        subject { helper.link_to_confirmation_required(@item.id, false, tag: "tagtag" ) }
        it { should ==  link_to(I18n.t('label.no_confirmation_mark').html_safe, tag_entry_confirmation_required_path("tagtag", @item.id, confirmation_required: true), remote: true, method: :put, class: 'item_confirmation_not_required') }
      end
    end
    ...
    context "when mark is specified," do
      context "when now confirmation_required is true," do
        ...
        it { should ==  link_to(I18n.t('label.confirmation_mark').html_safe, mark_entry_confirmation_required_path("markmark", @item.id, confirmation_required: false), remote: true, method: :put, class: 'item_confirmation_required') }
      end

      context "when now confirmation_required is false," do
        subject { helper.link_to_confirmation_required(@item.id, false, mark: "markmark" ) }
        it { should ==  link_to(I18n.t('label.no_confirmation_mark').html_safe, mark_entry_confirmation_required_path("markmark", @item.id, confirmation_required: true), remote: true, method: :put, class: 'item_confirmation_not_required') }
      end
    end
    ...
    context "when keyword is specified," do
      context "when now confirmation_required is true," do
        ...
        it { should ==  link_to(I18n.t('label.confirmation_mark').html_safe, keyword_entry_confirmation_required_path("keykey", @item.id, confirmation_required: false), remote: true, method: :put, class: 'item_confirmation_required') }
      end

      context "when now confirmation_required is false," do
        subject { helper.link_to_confirmation_required(@item.id, false, keyword: "keykey" ) }
        it { should ==  link_to(I18n.t('label.no_confirmation_mark').html_safe, keyword_entry_confirmation_required_path("keykey", @item.id, confirmation_required: true), remote: true, method: :put, class: 'item_confirmation_not_required') }
      end
    end
  end
