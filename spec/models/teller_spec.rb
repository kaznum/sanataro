  ...
  describe "#create_entry" do
    ...
    context "with confimation_required == true," do
      before do 
        ...
        @item, @affected_items, @is_error = Teller.create_entry(:action_date => Date.today, :name => 'テスト', :amount => 10000, :from_account_id => accounts(:bank1).id, :to_account_id => accounts(:outgo3).id, :user => users(:user1), :tag_list => 'hoge fuga', :confirmation_required => true)
      end
    ...
    context "with confimation_required is nil," do
      before do 
        @prev_count = Item.count
        @item, @affected_items, @is_error = Teller.create_entry(:action_date => Date.today, :name => 'テスト', :amount => 10000, :from_account_id => accounts(:bank1).id, :to_account_id => accounts(:outgo3).id, :user => users(:user1), :tag_list => 'hoge fuga')
      end
    ...
    context "about relation of created item, adjustment, pl," do
      ...
      context "created before adjustment which is in the same month," do
        before do
          @create = lambda { @item, @affected_items, @is_error = Teller.create_entry(:action_date => @init_adj2.action_date - 1, :name => 'テスト10', :amount => 10000, :from_account_id => accounts(:bank1).id, :to_account_id => accounts(:outgo3).id, :user => users(:user1)) }
        end
        ...
        describe "affected_items" do
          ...
          describe "the number of items" do 
            subject {  @affected_items }
            it { should have(1).items }

          end
          
          describe "affected item" do 
            subject {  @affected_items[0] }
            it { should == Item.find(items(:adjustment2).id) }
          end
        end
      ...
      context "created between adjustments which both are in the same month of the item to create," do
        before do
          @create = lambda { @item, @affected_items, @is_error = Teller.create_entry(:action_date => @init_adj4.action_date - 1, :name => 'テスト10', :amount => 10000, :from_account_id => accounts(:bank1).id, :to_account_id => accounts(:outgo3).id, :user => users(:user1)) }
        end
        ...
        describe "affected_items" do
          ...
          describe "affected item" do 
            subject {  @affected_items[0] }
            it { should == Item.find(items(:adjustment4).id) }
          end
        end
      ...
      context "created between adjustments, and the one is on earlier date in the same month and the other is in the next month of the item to create," do
        ...
        describe "affected_items" do
          ...
          describe "affected item" do 
            subject {  @affected_items[0] }
            it { should == Item.find(items(:adjustment6).id) }
          end
        end
      ...
      context "created between adjustments, and the one which is after item's date is in the same month and the other is in the previous month of the item to create," do
        before do
          @create = lambda { @item, @affected_items, @is_error = Teller.create_entry(:action_date => @init_adj6.action_date - 1, :name => 'テスト10', :amount => 10000, :from_account_id => accounts(:bank1).id, :to_account_id => accounts(:outgo3).id, :user => users(:user1)) }
        end
    ...
    context "about credit card payment," do
      ...
      context "created item with credit card, purchased before the settlement date of the month," do
        before do 
          @create = lambda { @item, @affected_items, @is_error = Teller.create_entry(:action_date => Date.new(2008,2,10), :name => 'テスト10', :amount => 10000, :from_account_id => accounts(:credit4).id, :to_account_id => accounts(:outgo3).id, :user => users(:user1)) }
        end
        ...
        describe "affected_items" do
          ...
          describe "affected item" do
            subject { @affected_items[0] }
            it { should == Item.where(:parent_id => @item.id).first }
          end
        end
      end
      ...
      context "created item with credit card, purchased before the settlement date of the month," do
        before do
          ...
          @create = lambda { @item, @affected_items, @is_error =
            Teller.create_entry(:action_date => Date.new(2008,2,25),
      ...
      context "created item with credit card, whose settlement_date == 99," do
        before do
          ...
          @create = lambda { @item, @affected_items, @is_error =
            Teller.create_entry(:action_date => Date.new(2008,2,10),
                                :name => 'テスト10', :amount => 10000,
                                :from_account_id => accounts(:credit4).id,
                                :to_account_id => accounts(:outgo3).id,
                                :user => users(:user1)) }
